...
class BreakDownProcessor:
    ...
    def process_query_count(self, query_content, query_count):
        ...
        if query_content is not None:
            if re.search(r"(AQ|GQ)(([0-9]+)|XXX|XX|X|)\:", query_content, re.IGNORECASE):
                ...
                query_content = re.sub(r"(AQ|GQ)(([0-9]+)|XXX|XX|X|)\:", new_qu, query_content, flags=re.IGNORECASE)
    ...
    def create_breakdown_document(self, jid, aid, docxfile):
        ...
        if self.journal_data[bjid]["BreakDown"] is True:
            ...
            # 1. issue-section (primary)
            # 2. article-type (if issue-section is missing/empty)
            # 3. other-article-type (if article-type is "Other")
            _issue_section = article_data['article_info'].get('issue-section') or ''
            _issue_section = _issue_section.strip() if _issue_section else ''

            if _issue_section:
                article_type = _issue_section
            else:
                _raw_article_type = article_data['article_info'].get('article-type') or ''
                _raw_article_type = _raw_article_type.strip() if _raw_article_type else ''
                if _raw_article_type.lower() == 'other':
                    # article-type is "Other" — use other-article-type
                    _other_article_type = article_data['article_info'].get('other-article-type') or ''
                    _other_article_type = _other_article_type.strip() if _other_article_type else ''
                    if _other_article_type and _other_article_type.lower() not in ('other', 'none'):
                        article_type = _other_article_type
                    else:
                        article_type = _raw_article_type
                elif _raw_article_type:
                    article_type = _raw_article_type
                else:
                    article_type = None
            ...
            if len(aut_dict) > 0:
                if len(new_authors_info) == len(aut_labeled):
                    ...
                    for author_key in updated_author_info:
                        ...
                        if re.search("([a-z0-9])", author['orcid']) and str(author.get('orcid-validated', '')).lower() == 'true':
                            orcid_found = True
            ...
            if rrh_line:
                ...
                if re.search("Surname", rrh_line, re.I):
                    rrh_line = re.sub("Surname", auth_rhead, rrh_line, flags=re.I)
            ...
            if bool(re.search("HISTORY", seq, re.I)) is True and "DR" in breakdown_dic:
                ...
                for paragraph in paragraphs:
                    ...
                    for orcid in breakdown_dic['Orcid_Para']:
                        if re.match(re.escape(orcid), paragraph.text, re.I) and para_style != "Orcid_Para":
                            paragraph.clear()
    ...
    def apply_label_styles(self, file_name):
        ...
        # Ensure required styles exist before labeling
        from docx.enum.style import WD_STYLE_TYPE
        for _sty_name in ("CPB", "CL", "EN", "ABKWH"):
            if _sty_name not in styles:
                try:
                    styles.add_style(_sty_name, WD_STYLE_TYPE.PARAGRAPH)
                except Exception:
                    pass
        for paragraph in document.paragraphs:
        ...
        for paragraph in document.paragraphs:
            ...
            if re.match("AT|H1|H2|H3|H4|EH", styleName, re.IGNORECASE):
                ...
                if not re.search("[a-z]", paraText) and non_ascii_found is False:
                    ...
                    if re.match(re.escape(paraText), title_text, re.IGNORECASE):
    ...
    def extract_authors_from_xml(self, input_string):
        ...
        input_string = re.sub(r" and | \& ", " ", input_string, flags=re.I)
        ...
        try:
            ...
            if author_found:
                ...
                if trans_msg.returncode == 0:
                    ...
                    html_content = re.sub(", ,", ", ", html_content, flags=re.I)
                    html_content = re.sub(",,", ",", html_content, flags=re.I)
                    html_content = re.sub(r"([a-z]) (\*)", r"\1\2", html_content, flags=re.I)
            ...
            # Clean up temp files — wrap each operation so cleanup
            # failures don't mask the real error or prevent return.
            try:
                if os.path.exists('temp/word/document.xml'):
                    os.remove('temp/word/document.xml')
            except Exception as _cleanup_err:
                print(f"[label_docx_authors] Cleanup warning: {_cleanup_err}")
            try:
                if os.path.exists(tmp_file):
                    os.replace(tmp_file, docx_file_path)
            except Exception as _replace_err:
                print(f"[label_docx_authors] File replace warning: {_replace_err}")
    ...
    def bd_preclean(self, docx_file_path):
        ...
        # Remove f1000-at-ignore character style references from runs.
        # The style is wrapped around runs as <w:rStyle w:val="f1000-at-ignore"/>
        # inside <w:rPr>...</w:rPr>. We just strip the rStyle tag — keep the run
        # and its text intact so content is preserved without the style.
        document_xml = re.sub(
            r'<w:rStyle\s+w:val="f1000-at-ignore"\s*/>',
            '',
            document_xml,
            flags=re.I
        )
        # Also clean up empty <w:rPr></w:rPr> that may result from the removal
        document_xml = re.sub(
            r'<w:rPr>\s*</w:rPr>',
            '',
            document_xml,
    ...
    def process_author_line(self, labeled_author):
        ...
        labeled_author = re.sub(r'<au([^>]+|)>(\,|\.|\;| and |and | \&|\&|)(\s+|)', r"<au\1>",
                                labeled_author, flags=re.I | re.S | re.DOTALL)
        labeled_author = re.sub(r'<au([^>]+|)>and ', r"<au\1>", labeled_author, flags=re.I | re.S | re.DOTALL)
        labeled_author = re.sub(r'<au([^>]+|)>\, and ', r"<au\1>", labeled_author, flags=re.I | re.S | re.DOTALL)
        labeled_author = re.sub(r'<au([^>]+|)>\. and ', r"<au\1>", labeled_author, flags=re.I | re.S | re.DOTALL)
        labeled_author = re.sub(r'<au([^>]+|)>\; and ', r"<au\1>", labeled_author, flags=re.I | re.S | re.DOTALL)
        labeled_author = re.sub(r'<au([^>]+|)>\: and ', r"<au\1>", labeled_author, flags=re.I | re.S | re.DOTALL)
        labeled_author = re.sub(r'<au([^>]+|)>\, \& ', r"<au\1>", labeled_author, flags=re.I | re.S | re.DOTALL)
        labeled_author = re.sub(r'<au([^>]+|)>\. \& ', r"<au\1>", labeled_author, flags=re.I | re.S | re.DOTALL)
        labeled_author = re.sub(r'<au([^>]+|)>\; \& ', r"<au\1>", labeled_author, flags=re.I | re.S | re.DOTALL)
        labeled_author = re.sub(r'<au([^>]+|)>\: \& ', r"<au\1>", labeled_author, flags=re.I | re.S | re.DOTALL)
