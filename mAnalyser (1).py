    ...
    def create_info_file(self, customer, file_dics, jid, aid, ms_no, unique_id):
        if re.match("^SAGE$", customer):
            ...
            if file_dics['long_metadata'] is None:
                ...
                # into the merger output folder (ready for ParaStyler).
                # No merge logic needed — just proper file naming.
                has_short_meta_only = (
                ...
                if has_short_meta_only:
                    ...
                    dest_file = os.path.join(merger_input, cln_name)
                    ...
                    print(f"[INFO] Short-metadata only package with single docx. "
                          f"Renamed '{single_docx_name}' → '{cln_name}', skipping merge logic.")
