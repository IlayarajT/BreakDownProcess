...
class BreakDownWorker(QThread):
    ...
    def _execute(self):
        ...
        try:
            ...
            print(f"[BreakDown] Regex error traceback:\n{tb_str}")
            self.finished.emit(
                False,
                f"ERROR: Regex pattern error during BreakDown:\n{regex_err}\n\n"
                f"This may be caused by special characters in headings or titles.\n"
                f"File: {file_full_path}\n\n"
                f"Traceback (check console for full details):\n"
                f"{tb_str[-300:]}"
            )
            return
            ...
            self.finished.emit(
                False,
                f"ERROR: Missing data key during BreakDown: {key_err}\n\n"
                f"Check that the JSON file has all required fields.\n"
                f"File: {file_full_path}"
            )
            return
            ...
            self.finished.emit(
                False,
                f"ERROR: BreakDown process failed:\n{type(bd_err).__name__}: {bd_err}\n\n"
                f"File: {file_full_path}"
            )
            return
...
class ParaStylerWorker(QThread):
    ...
    def _execute(self):
        ...
        # ── Step 2: Run Para Styler (mode decided by paraStylerLocation.yml) ──
        self.status.emit("Reading Para Styler configuration...")
        self.progress.emit(10)

        # paraStylerLocation.yml lives next to BreakDown.exe (NOT in _internal).
        # In frozen mode sys.executable points to BreakDown.exe, so its
        # dirname is the exe folder.  In dev mode fall back to app_path.
        import yaml as _yaml
        if getattr(sys, 'frozen', False):
            _exe_dir = os.path.dirname(sys.executable)
            _local_styler_bat = "local_styler.bat"
        else:
            _exe_dir = app_path
            _local_styler_bat = "local_styler_dev.bat"

        parastyler_config_path = os.path.join(_exe_dir, "paraStylerLocation.yml")
        if not os.path.exists(parastyler_config_path):
            self.finished.emit(False, "ERROR: paraStylerLocation.yml not found at: " + parastyler_config_path)
            return

        with open(parastyler_config_path, "r") as _wf:
        ...
        if inbuild_parastyler:
            ...
            if processresult is False:
                ...
                self.progress.emit(20)
                styler_path = os.path.join(_exe_dir, _local_styler_bat)
        ...
        # ── 2B: WATCHER / HOT-FOLDER mode ──────────────────────────────
            ...
            # ── Pre-check: is the watcher already busy? ──────────────────
            # If another file is already in the process folder, wait up to
            # 30 seconds for it to finish.  If still busy, pull back our
            # file from watcher input and fall back to the local JAR.
            ...
            if _watcher_busy:
                self.status.emit("Watcher busy — waiting 30 s for it to finish...")
                self.progress.emit(16)
                _busy_wait   = 30
                _busy_poll   = 2
                _busy_elapsed = 0
                while _busy_elapsed < _busy_wait:
                    time.sleep(_busy_poll)
                    _busy_elapsed += _busy_poll
                    _remaining = _busy_wait - _busy_elapsed
                    self.status.emit("Watcher busy — waiting (%d s remaining)..." % _remaining)
                    # Re-check
                    _proc_files = [
                        f for f in os.listdir(watcher_process)
                        if os.path.isfile(os.path.join(watcher_process, f))
            ...
            # ── Helper: local JAR fallback ────────────────────────────────
            # Used when watcher is busy, stalled, or timed out.
            def _run_local_fallback():
                """Clean up watcher input and run local styler. Returns as_file path or None."""
                ...
                self.status.emit("Running local Para Styler (fallback)...")
                self.progress.emit(25)
                ...
                if _local_ok is False:
                    ...
                    _sty_path = os.path.join(_exe_dir, _local_styler_bat)
                    _p = Popen([_sty_path, file_full_path], stdout=PIPE, stderr=PIPE)
                    _local_as = re.sub(".docx", "_AS.docx", file_full_path)
                    _p.communicate()
                    _p.wait()
            ...
            if _watcher_busy:
                # Watcher is still occupied after 30 s — fall back to local JAR
                as_file = _run_local_fallback()
                if not os.path.exists(as_file):
                    self.finished.emit(False, "ERROR: AS file not created (local fallback). Please proceed manually.")
                    return

            else:
                # ── Normal hot-folder polling (watcher is free) ────────────
                timeout_seconds = 180
                poll_interval   = 2
                elapsed         = 0
                ...
                result_found = None   # "completed" | "error" | "stall" | None (timeout)
                ...
                if result_found == "stall" or result_found is None:
                    # Watcher stalled or timed out — fall back to local JAR
                    _reason = "stalled" if result_found == "stall" else "timed out"
                    self.status.emit("Watcher %s — falling back to local Para Styler..." % _reason)
                    as_file = _run_local_fallback()
                    if not os.path.exists(as_file):
                        self.finished.emit(False, "ERROR: AS file not created (local fallback after watcher %s). Please proceed manually." % _reason)
                        return

                elif result_found == "completed":
...
class Ui_Dialog(QWidget):
    ...
    def run_para_styler(self, doc_file):
        ...
        if getattr(sys, 'frozen', False):
            bat_name = "local_styler.bat"
        else:
            bat_name = "local_styler_dev.bat"
        styler_path = os.path.join(self.app_path, bat_name)
        QApplication.setOverrideCursor(Qt.CursorShape.WaitCursor)
