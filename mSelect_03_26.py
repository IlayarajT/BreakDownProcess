...
class ParaStylerWorker(QThread):
    ...
    def _execute(self):
        ...
        # ── 2B: WATCHER / HOT-FOLDER mode ──────────────────────────────
            ...
            # JSON file: <jrn_id>_<art_id>.json must already be in the article folder
            json_name   = jrn_id + "_" + art_id + ".json"
            json_source = os.path.join(file_folder, json_name)
            if not os.path.exists(json_source):
                self.finished.emit(False, "ERROR: JSON file not found: " + json_source)
                return
            ...
            # Copy JSON first, then DOCX (watcher triggers on DOCX arrival)
