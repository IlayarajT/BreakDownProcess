...
class BreakDownProcessor:
    ...
    def create_breakdown_document(self, jid, aid, docxfile):
        ...
        if self.journal_data[bjid]["BreakDown"] is True:
            ...
            _raw_article_type = article_data['article_info'].get('article-type', '')
            if not _raw_article_type or _raw_article_type.strip().lower() == 'other':
                # article-type is missing or "Other" — try other-article-type first
                _other_article_type = article_data['article_info'].get('other-article-type', '')
                if _other_article_type and _other_article_type.strip().lower() not in ('', 'other', 'none'):
                    article_type = _other_article_type.strip()
                else:
                    article_type = article_data['article_info'].get('issue-section')
            else:
