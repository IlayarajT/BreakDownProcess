...
def insert_labels_into_html(html: str, authors: List[AuthorEntity]) -> str:
    ...
    has_any_sup = bool(re.search(r"<sup>", html))
    ...
    while i < len(parts):
        ...
        if chunk.startswith("<sup>"):
            ...
            if not next_is_sup and au_idx_seen < len(authors):
                # Text chunk without a following sup — this is a trailing/last author
                # Place label after any trailing markers at the end of the chunk
                text = chunk
                # Strip trailing markers from end of text, place label after them
...
def _insert_labels_no_sup(html: str, authors: List[AuthorEntity]) -> str:
    """
    Insert labels into HTML that has no <sup> tags.
    Find each author's name in the text and insert the label after it.
    ...
    # Build list of (position, length, label) for each author
    insertions: List[Tuple[int, str]] = []  # (position_after_name, label_text)
    ...
    for a in authors:
        ...
        # Find the name in the plain text
        # Try exact match first
        pos = -1
        search_start = max(used_positions) + 1 if used_positions else 0
        ...
        if idx >= 0:
            ...
            if idx >= 0 and idx not in used_positions:
                pos = idx + len(name)
        
        if pos >= 0:
            used_positions.add(idx)
            insertions.append((pos, label))
    
    if not insertions:
        # Fallback: just append all labels at the end
        return html + "".join(
            f'<span class="aulabel">[AU{a.index}]</span>' for a in authors
        )
    
    # Sort insertions by position (should already be in order)
    insertions.sort(key=lambda x: x[0])
    
    # Build result by inserting labels at the right positions in the original html
    # Since html == plain (no tags in no-sup case), positions map directly
    result = []
    last_pos = 0
