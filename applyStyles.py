...
class ApplyStyles:
    ...
    def apply_styles(self, docxfile):
        ...
        document = self.detect_list_styles(document)
    ...
    # ------------------------------------------------------------------
    #  detect_list_styles
    #  Classifies list paragraphs into three styles:
    #    BL  — bullet glyph (•, ▪, Wingdings etc.) first character
    #          or Word numFmt='bullet' with a glyph lvlText
    #    NL  — numbered: decimal / roman / alpha with any bracket/dot:
    #          1. (1) 1) a. (a) i. (i) etc.
    #    UL  — tab/plain list: no bullet glyph, no number prefix;
    #          indented with tab or Word list-paragraph style
    # ------------------------------------------------------------------
    def detect_list_styles(self, document):
        """Assign BL, NL, or UL to list paragraphs."""

        styles = document.styles
        for sty in ("BL", "NL", "UL"):
            if sty not in styles:
                document = self.create_style(document, sty)

        # ── numbering part (may not exist in this document) ───────────
        numbering_part = None
        try:
        ...
        NUMBERED_FORMATS = {
            ...
            'decimalEnclosedParen', 'decimalEnclosedFullstop',
            'decimalFullWidth', 'decimalHalfWidth',
            'hex', 'ganada', 'chosung',
        }
        UL_STYLE_NAMES = {
            'List Paragraph', 'listtabbed', 'listunordered',
            'lista', 'listb', 'listc', 'listd', 'liste', 'listf',
            'listg', 'listh', 'listitem', 'liststart', 'listend',
            'listastart', 'listaend', 'listbstart', 'listbend',
            'listcstart', 'listcend', 'listdstart', 'listdend',
        }
        # Styles that belong to the references/bibliography section —
        # these must never be reclassified as BL / NL / UL.
        REFERENCE_STYLE_NAMES = {
            'bibitem', 'bib', 'bibl', 'biblio', 'bibliography',
            'ref', 'refs', 'reflist', 'reference', 'references',
            'refitem', 'refpara', 'reftext',
        }

        def _is_reference_style(style_name):
            """Return True if the style name matches any known reference style."""
            return style_name.lower() in REFERENCE_STYLE_NAMES

        NL_TEXT_RE = re.compile(
            r'^[\(\[]?[0-9ivxlIVXLa-zA-Z]{1,4}[.)\}\]]\s'
        )

        def _classify_by_numpr(num_id_val):
            if numbering_part is None:
                return ''
            nums = numbering_part._element.xpath(
                f'w:num[@w:numId="{num_id_val}"]', namespaces=nsmap)
            if not nums:
                return ''
            abstract_ref = nums[0].xpath('w:abstractNumId/@w:val', namespaces=nsmap)
            if not abstract_ref:
                return ''
            abstract_nums = numbering_part._element.xpath(
                f'w:abstractNum[@w:abstractNumId="{abstract_ref[0]}"]',
                namespaces=nsmap)
            if not abstract_nums:
                return ''
            an = abstract_nums[0]
            fmt_vals = an.xpath('w:lvl[@w:ilvl="0"]/w:numFmt/@w:val', namespaces=nsmap)
            txt_vals  = an.xpath('w:lvl[@w:ilvl="0"]/w:lvlText/@w:val', namespaces=nsmap)
            fmt      = fmt_vals[0] if fmt_vals else ''
            lvl_text = txt_vals[0]  if txt_vals  else ''

            if fmt == 'bullet':
                if lvl_text in UL_LVLTEXT:
                    return 'UL'
                stripped = lvl_text.strip()
        ...
        # ── First pass: paragraphs with <w:numPr> ─────────────────────
        for paragraph in document.paragraphs:
            # Never reclassify reference / bibliography paragraphs
            if _is_reference_style(paragraph.style.name):
                continue
            pPr = paragraph._p.pPr
            if pPr is None:
                continue
            ...
            if numId_el is None:
                continue
            num_id_val = numId_el.get(qn('w:val'), '0')
            if num_id_val == '0':
                continue
            target = _classify_by_numpr(num_id_val)
            if target in ('BL', 'NL', 'UL'):
                paragraph.style = document.styles[target]

        # ── Second pass: no numPr — detect by content / style name ────
        for paragraph in document.paragraphs:
            text = paragraph.text
            if not text:
                continue
            ...
            # Never reclassify reference / bibliography paragraphs
            if _is_reference_style(current_style):
                continue
            ...
            # Only apply bullet-glyph detection (visually unambiguous).
            # The NL_TEXT_RE check is intentionally skipped for non-list
            # styles to prevent reference entries like "[1] Author…" from
            # being mis-classified as NL.

            # Bullet glyph as first character → BL
            if first_char in BULLET_GLYPHS:
                paragraph.style = document.styles["BL"]
                continue

            # Tab-indented with a recognised list-adjacent style → UL
            if first_char == '\t':
                if current_style in ('NL', 'BL', 'UL', 'List Paragraph') or \
                   current_style in UL_STYLE_NAMES:
                    paragraph.style = document.styles["UL"]
    ...
    # ------------------------------------------------------------------
    #  bm_named_styles
    #  Converts every explicit backmatter style written by breakDownProcess
    #  to the correct output styles:
    #      *_Title  →  EH
    #      *_Para   →  AN
    # ------------------------------------------------------------------
    BM_TITLE_STYLES = {
    ...
    def bm_named_styles(self, document):
        """
        Walk all paragraphs; any paragraph whose style name is in
        BM_TITLE_STYLES is re-styled to EH, any in BM_PARA_STYLES
        is re-styled to AN.  EH and AN are created first if absent.
        """
        styles = document.styles
        if "EH" not in styles:
            document = self.create_style(document, "EH")
        ...
        for paragraph in document.paragraphs:
            style_name = paragraph.style.name
            if style_name in self.BM_TITLE_STYLES:
                paragraph.style = document.styles["EH"]
            elif style_name in self.BM_PARA_STYLES:
                paragraph.style = document.styles["AN"]
